...
const TrainingGamificationApp = () => {
  ...
  const startModule = (m) => {
    if (m.locked) { showReward("🔒 Desbloquea completando más módulos"); return; }
    setSelectedModule(m); setCurrentView("module"); setQuizActive(false); setCurrentQuestion(0); setQuizScore(0);
  };
  const startQuiz = () => { setQuizActive(true); setCurrentQuestion(0); setQuizScore(0); };
  const answerQuestion = (i) => {
    if (i === quizQuestions[currentQuestion].correct) { setQuizScore(quizScore + 1); showReward("✅ ¡Correcto! +10 XP"); }
    else { showReward("❌ Incorrecto, sigue intentando"); }
    if (currentQuestion < quizQuestions.length - 1) setTimeout(() => setCurrentQuestion(currentQuestion + 1), 1000);
    else setTimeout(() => completeModule(), 1500);
  };
  ...
  const Notification = () => (
    <div className={`fixed top-4 right-4 z-[60] transition-all transform ${showNotification ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"}`}>
      <div className="glass px-6 py-4 flex items-center gap-3 ring-1 ring-indigo-400/40">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/90">{userData.position} • {userData.department}</p>
              </div>
            </div>
              ...
              <div key={idx} className="glass p-4">
                ...
                {box.bar != null && (
                  <div className="mt-2 bg-white/10 rounded-full h-2">
                    <div className="bg-gradient-to-r from-yellow-300 to-pink-300 h-2 rounded-full" style={{ width: `${box.bar}%` }} />
                  </div>
                )}
              </div>
            ))}
  ...
  const ModuleView = () => {
    ...
    return (
      <div className="pb-[96px]">{/* reserva fija para el navbar */}
          ...
          {!quizActive ? (
            <div className="glass p-6">
              ...
              <div className="border-t border-white/10 pt-6">
                ...
                <div className="rounded-xl aspect-video bg-white/10 flex items-center justify-center mb-6">
            ...
            <div className="glass p-6">
              <div className="mb-6">
                ...
                <div className="bg-white/10 rounded-full h-3">
                  <div className="bg-gradient-to-r from-indigo-400 to-fuchsia-400 h-3 rounded-full transition-all" style={{ width: `${((currentQuestion + 1) / quizQuestions.length) * 100}%` }} />
                </div>
              </div>
              <div className="mb-8">
        ...
        {/* Spacer extra por si algún contenedor llega al borde inferior */}
  ...
  const BottomNavigation = () => (
    <div
      className="fixed bottom-0 left-0 right-0 z-50 backdrop-blur bg-black/40 border-t border-white/10 shadow-[0_-10px_30px_-15px_rgba(0,0,0,.5)]"
      style={{ paddingBottom: "env(safe-area-inset-bottom)" }}
    >
      <div className="neo-container">
        <div className="flex justify-around py-3 min-h-[72px]">
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center p-2 rounded-lg transition-colors ${
                currentView === tab.key ? "text-fuchsia-300" : "text-slate-400"
              }`}
            >
              {tab.icon}
